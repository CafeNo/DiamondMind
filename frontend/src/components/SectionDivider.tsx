...
const SectionDivider = ({
  ...
  if (variant === "wave") {
    return (
      <div
        ref={dividerRef}
        className={`w-full ${heightClasses[height]} relative z-50 -mt-32 opacity-0 transition-opacity duration-1000 overflow-visible`}
      >
        <svg
        ...
        <div className="absolute top-4 left-1/4 w-1 h-1 bg-shirin-blue rounded-full opacity-40"></div>
        <div
          className="absolute top-6 right-1/3 w-1 h-1 bg-shirin-pink rounded-full opacity-30"
        ></div>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center space-x-8">
          <div className="w-8 h-8 bg-gradient-to-r from-shirin-blue to-shirin-purple rounded-full opacity-60"></div>
          <div className="w-6 h-6 bg-gradient-to-r from-shirin-purple to-shirin-pink transform rotate-45 opacity-70"></div>
          <div className="w-10 h-10 border-2 border-shirin-blue rounded-full opacity-50"></div>
        </div>
    ...
    return (
      ...
      >
        {[...Array(8)].map((_, i) => (
          <div
            key={i}
            className="absolute w-0.5 h-0.5 bg-shirin-blue rounded-full opacity-40"
            style={{
        ...
        {[...Array(5)].map((_, i) => (
          <div
            key={`purple-${i}`}
            className="absolute w-1 h-1 bg-shirin-purple rounded-full opacity-30"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
            }}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-8 h-8 bg-shirin-blue/10 rounded-full blur-sm"></div>
        <div className="w-6 h-6 bg-shirin-purple/15 rounded-full blur-sm ml-4"></div>
      </div>
    </div>
  );
};
